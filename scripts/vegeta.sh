...
source "${SCRIPT_DIR}/vegeta-runner.sh"

# Vegeta-specific configuration
RESULTS_DIR="${RESULTS_DIR:-$RESULTS_DIR_VEGETA}"
...
main() {
  print_separator
  print_message "$GREEN" "Starting ${SERVER} Load Test with Vegeta"
  print_separator
  ...
  for rate in "${RATES[@]}"; do
    ...
    run_test_for_rate "$rate" "$current_test" "$total_tests"
  done
  ...
  for rate in "${RATES[@]}"; do
    print_message "$GREEN" "  - $RESULTS_DIR/${rate}rps.bin"
    print_message "$GREEN" "  - $RESULTS_DIR/${rate}rps.json"
    print_message "$GREEN" "  - $RESULTS_DIR/${rate}rps-cpu.csv"
  done
  print_separator
}
